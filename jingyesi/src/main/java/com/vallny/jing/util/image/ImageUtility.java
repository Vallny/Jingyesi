package com.vallny.jing.util.image;
...
import com.vallny.jing.util.file.FileLocationMethod;
import com.vallny.jing.util.file.FileManager;
import com.vallny.jing.util.global.Utility;
import com.vallny.jing.util.http.FileDownloaderHttpHelper;
...
public class ImageUtility {
    ...
    private static int calculateInSampleSize1(Options options, int reqWidth, int reqHeight) {
    ...
    public static boolean getBitmapFromNetWork(String url, String path, FileDownloaderHttpHelper.DownloadListener downloadListener) {
        for (int i = 0; i < 3; i++) {
            if (HttpUtility.getInstance().doGetSaveFile(url, path, downloadListener)) {
                return true;
            }
